...
# monitor.sh - Monitor de recursoso del sistema
...
uso() {
	...
	echo "	--umbral-disco N	Alerta de disco %"
...
# Escribe al log y a pantall con marca de tiempo
registrar() {
	local nivel="$1"
	local mensaje="$2"
	local ts
...
while true; do
	...
	# Salir si se alcanza el limite de iteraciones
	if [ "$MAX_ITER" -gt 0 ] \
	...
	[ "$disco" -ge "$UMBRAL_DISCO" ] && registrar "ALERTA" \
		"Disco al ${disco}% (umbral:${UMBRAL_DISCO}%)"

	[ "$ram" -ge "$UMBRAL_RAM" ] && registrar "ALERTA" \
