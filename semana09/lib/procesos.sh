...
# Lista los N procesos con uso de memoria
listar_top_mem() {
        local n="${1:-5}"
...
# Verificar si un proceso esta activo
verificar_proceso() {
	local nombre="$1"
	if pgrep -x "$nombre" > /dev/null 2>&1; then
		local pid
		pid=$(pgrep -x "$nombre" | head -1)
		log "OK" "Procesos '$nombre' activo (PID: $pid)"
		return 0
	else
...
# Muestra el arbol de proceso de un usuario
arbol_usuario() {
