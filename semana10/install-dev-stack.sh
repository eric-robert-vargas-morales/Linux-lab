...
# Instala y configura el dev stacj del curso Linux
...
instalar_paquete() {
	...
	if dpkg -l "$paquete" &>/dev/null; then
		...
		log OK "$descripcion ya instalada (v$version) -- omitidos"
...
instalar_dev_stack() {
	...
	if ! $DRY_RUN; then
		...
		log OK "Lista de paquetes actualizado"
	fi
	...
	instalar_paquete "vim"		"Vim (visualizar directorios)"
	instalar_paquete "tree"		"tree (visualizar directorio)"
